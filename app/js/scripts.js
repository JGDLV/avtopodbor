...
$(document).ready(function () {
  ...
  $('.parameters__item').first().addClass('active');
  $('.parameters__item').first().find('.parameters__item-text-part').css('display', 'block');
  ...
  $(window).on('load resize', function () {
    ...
    if ($(window).width() < mobileBreakpoint && $('.menu-toggle').length === 0) {
      $('.menu').prepend(mobileMenu);
    } else if ($(window).width() > mobileBreakpoint && $('.menu-toggle').length === 1) {
      $('.menu-toggle').remove();
    }
  ...
  $(window).scroll(function () {
    $(this).scrollTop() > headerHeight
      ...
      : ($('.menu').removeClass('stickytop'), $('body').css('padding-top', '0'));
  });

  $('.we-check__item').first().addClass('active');
  $('.we-check__right-item').first().css('display', 'block');

  $('.we-check__item').on('click', function() {
    const item = $(this);
    item.siblings().removeClass('active');
    item.addClass('active');
    ...
    $('.we-check__right-item').hide();
    $('.we-check__right-item').eq(index).fadeIn(300);
  });
